...
// Данные пакетов UC хранятся прямо в переменной
let packages = [
...
app.post('/api/update-price', (req, res) => {
  const { password, id, newPrice } = req.body;
  if (password !== 'supersecret') {
    res.json({ success: false, error: 'Неверный пароль' });
    return;
  }
  if (typeof id !== 'number' || typeof newPrice !== 'number' || newPrice <= 0) {
    res.json({ success: false, error: 'Неверные данные' });
    return;
  }
  const pack = packages.find(p => p.id === id);
  if (!pack) {
    res.json({ success: false, error: 'Пакет не найден' });
    return;
  }
  pack.price = newPrice;
